...
void ILI9488_Lcd_Scan_Dir(uint16_t width, uint16_t height, uint16_t Xcmd, uint16_t Ycmd, uint8_t dir)
{
  uint16_t regval = 0;
  uint16_t dirreg = 0;
...
void ILI9488_Lcd_Color_Fill(uint16_t Xcmd, uint16_t Ycmd, uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, const uint16_t *color)
{
  LCD_WR_REG(Xcmd);
  LCD_WR_DATA(sx >> 8);
  LCD_WR_DATA(sx & 0xff);
...
uint16_t ILI9488_Get_IC_ID(void)
{
  uint16_t ic_id = 0;
  LCD_WR_REG(0XD3);
  (void)LCD_RD_DATA();
