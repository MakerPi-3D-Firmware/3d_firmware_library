...
namespace gcode
{
  static volatile float arc_target[MAX_NUM_AXIS];
  static volatile uint32_t previous_millis_cmd = 0U;
  static volatile float offset[XYZ_NUM_AXIS] = {0.0F};

  static void mc_arc(const volatile float (&position)[MAX_NUM_AXIS], const volatile float (&target)[MAX_NUM_AXIS],
                     const volatile float (&offset)[XYZ_NUM_AXIS], uint8_t axis_0, uint8_t axis_1,
                     uint8_t axis_linear, float feed_rate, float radius, uint8_t isclockwise, uint8_t extruder)
  {
    ...
    */
    ...
    for (i = 1; i < segments; ++i) // Increment (segments-1)
    {
      ...
      gcode::active_extruder = extruder;
  ...
  {
#ifdef SF_ARC_FIX
    bool relative_mode_backup = planner_settings.relative_mode;
    planner_settings.relative_mode = true;
#endif
    get_coordinates();
#ifdef SF_ARC_FIX
    planner_settings.relative_mode = relative_mode_backup;
#endif
    ...
    if (flash_param_t.extruder_type == EXTRUDER_TYPE_LASER)
    {
      ...
      {
        offset[X2_AXIS] = parseGcodeBufHandle.codeValue();
      }
      else
      {
        offset[X2_AXIS] = 0.0F;
    ...
    else
    {
      if (parseGcodeBufHandle.codeSeen('I'))
      {
        offset[X_AXIS] = parseGcodeBufHandle.codeValue();
      }
      else
      {
        offset[X_AXIS] = 0.0F;
    ...
    {
      offset[Y_AXIS] = parseGcodeBufHandle.codeValue();
    }
    else
    {
      offset[Y_AXIS] = 0.0F;
  ...
  static void prepare_arc_move(const volatile uint8_t isclockwise)
  {
    ...
    if (flash_param_t.extruder_type == EXTRUDER_TYPE_LASER)
    {
      arc_target[X_AXIS] = ccm_param.t_model.xyz_home_pos[X_AXIS];
      r = hypot(offset[X2_AXIS], offset[Y_AXIS]); // Compute arc radius for mc_arc
      // Trace the arc
      mc_arc(ccm_param.grbl_current_position, ccm_param.grbl_destination, offset, X2_AXIS, Y_AXIS, Z_AXIS,
             ((feedrate * static_cast<float>(feedmultiply)) / 60.0F) / 100.0F, r, isclockwise, gcode::active_extruder);
    }
    else
    {
      arc_target[X2_AXIS] = ccm_param.t_model.xyz_home_pos[X2_AXIS];
      mc_arc(ccm_param.grbl_current_position, ccm_param.grbl_destination, offset, X_AXIS, Y_AXIS, Z_AXIS,
             ((feedrate * static_cast<float>(feedmultiply)) / 60.0F) / 100.0F, r, isclockwise, gcode::active_extruder);
    }
    ...
    for (uint8_t i = 0U; i < ccm_param.t_sys.axis_num; ++i)
    {
      ccm_param.grbl_current_position[i] = ccm_param.grbl_destination[i];
    }

    previous_millis_cmd = xTaskGetTickCount();
  }
